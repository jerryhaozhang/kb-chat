...
同一浏览器会话内保持多轮对话上下文，支持代词指代和追问。

**Session 生命周期：**
- 打开页面 → 生成 UUID 作为 sessionId（存 sessionStorage，关闭浏览器自动清除）
- 整个会话期间自动记忆所有对话
- 关闭浏览器标签页 → 会话结束，前端 sessionId 消失
- 服务端文件保留 7 天后自动清理

**记忆内容（三层组合）：**
1. **最近对话** — 最近 8 轮 Q&A 原文
2. **历史摘要** — 超出 8 轮的早期对话由 LLM 压缩为摘要
3. **用户画像** — 从对话中提取的偏好（酒店、日期、人数等）

**存储：**
- 服务端 `memory/{sessionId}.json`，每会话一个文件
- 请求 body 新增 `sessionId` 字段（前端自动附带）

**示例：**
```
第 1 轮: "8 月澳门出发 6 天 5 晚多少钱" → 系统返回报价
第 2 轮: "刚才那个行程再加一晚潜水多少钱" → 系统能引用上一轮的酒店和日期
```
...
- 会话上下文记忆仅在同一浏览器 tab 内有效，不支持跨设备/跨浏览器
