...
上游 Obsidian Vault 中 `潜水知识库/行程价格速查/帕劳/` 新增了国庆期间（9.29-10.2）全酒店价格表，覆盖 17 房型 × 6 日期/航班组合。原来的 `国庆假日酒店专属套餐` 仅覆盖假日酒店，现在扩展为全酒店。同时假日酒店旧套餐中 9.29-10.2 时段的条目已标记为过期。

相应更新了 `scripts/generate.js` 和生成的 `knowledge/` 文件。

### generate.js 改动

1. **`buildPriceData()`** — 新增解析 `酒店价格速查.md` 中 `## 国庆期间（9.29-10.2）` 章节的全酒店价格矩阵，输出 `holidayFull` 对象（columns + hotels）
2. **`buildPricePrompt()`** — system prompt 新增国庆全酒店价格表（最高优先级）；假日酒店专属套餐标注为 10.3-10.9 有效；更新查询优先级为 5 级
3. **`buildInfoPrompt()`** — 接收 `holidayFull` 参数，动态生成国庆全酒店价格表 + 航班信息；跳过已过期条目；移除硬编码的旧版假日酒店国庆价格
...
|--------|------|
| 国庆价格表 18 条目 × 6 列全部解析 | ✅ |
| 假日酒店过期条目过滤（仅保留 10.3-10.9 有效） | ✅ |
| 附加费规则标注国庆期间不叠加 | ✅ |
| `holidayFull` 写入 data.json | ✅ |

### 待部署

- 服务器端需 `git pull` + `pm2 restart kb-chat`
